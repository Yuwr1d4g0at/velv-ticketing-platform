...
function upsertAgent(name, email, departmentId, isAdmin = 0) {
  const existing = db.prepare("SELECT id FROM agents WHERE email = ?").get(email);
  if (existing) {
    db.prepare("UPDATE agents SET department_id = ?, is_admin = ? WHERE id = ?").run(departmentId, isAdmin, existing.id);
    return existing.id;
  }
  const result = db
...
function insertTicket({ subject, description, category, requesterName, requesterEmail, assignedTo, confidential = 0 }) {
  const result = db
    .prepare(
...
const hrDept = db.prepare("SELECT id FROM departments WHERE name = 'HR'").get().id;
...
const rodrigoId = upsertAgent("rodrigo", "rodrigo.dias@velv.pt", itDept, 0);
const teoId = upsertAgent("Teo Test", "teotesting@test.pt", itDept, 1);
const anaId = upsertAgent("Ana Ferreira", "ana.ferreira@velv.pt", hrDept, 0);

const itTicket1 = insertTicket({
