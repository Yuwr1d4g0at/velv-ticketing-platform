...
test("runSync() is a no-op when Microsoft Graph isn't configured", async () => {
  ...
  assert.equal(db.prepare("SELECT COUNT(*) c FROM asset_sync_runs").get().c, 0);
});

test("isDue() is true with no prior runs, and false right after a logged run within the interval", () => {
  const assetSync = require("../src/assetSync");
  assert.equal(assetSync.isDue(24), true);

  db.prepare(
    `INSERT INTO asset_sync_runs (started_at, finished_at, created_count, updated_count, failed_count) VALUES (datetime('now'), datetime('now'), 5, 2, 0)`
  ).run();
  assert.equal(assetSync.isDue(24), false);
  assert.equal(assetSync.isDue(0), true); // a 0-hour interval is always due
});
