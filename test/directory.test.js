...
function seedProfile(email, fields) {
  db.prepare(
...
test("a ticket page shows no directory enrichment when nothing is cached and Graph isn't configured", async () => {
  const ticketId = db
    ...
    .run("No enrichment yet", "desc", "Plain Requester", "plain.requester@example.com").lastInsertRowid;
...
test("a ticket page shows cached department/job title/phone for the requester", async () => {
  seedProfile("enriched.requester@velv.pt", { displayName: "Enriched Requester", department: "Finance", jobTitle: "Analyst", phone: "+351 912 345 678" });
  const ticketId = db
    .prepare(
      `INSERT INTO tickets (subject, description, category, requester_name, requester_email) VALUES (?, ?, 'Hardware', ?, ?)`
    )
    .run("Enriched ticket", "desc", "Enriched Requester", "enriched.requester@velv.pt").lastInsertRowid;
...
test("directory enrichment is skipped once a ticket's data has been erased (GDPR)", async () => {
  seedProfile("erased.requester@velv.pt", { displayName: "Erased Requester", department: "Should not appear" });
  const ticketId = db
    .prepare(
      `INSERT INTO tickets (subject, description, category, requester_name, requester_email, data_erased_at)
       VALUES (?, ?, 'Hardware', ?, ?, datetime('now'))`
    )
    .run("Erased ticket", "desc", "[erased]", "erased.requester@velv.pt").lastInsertRowid;
...
test("the Agents page shows each agent's cached department/job title when present", async () => {
  seedProfile("directory-agent@example.com", { department: "IT Department", jobTitle: "Helpdesk Lead" });
